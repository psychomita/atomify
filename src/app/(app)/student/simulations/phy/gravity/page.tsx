...
export default function GravitySimulation() {
  ...
  }, [isRunning, bodies, timeScale, collisionsEnabled]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const updateSelectedBody = (property: string, value: string | number) => {
    if (!selectedBody) return;
    ...
    setBodies(
      bodies.map((body) => {
        if (body.id === selectedBody) {
          if (property === "name") {
            return { ...body, name: value as string };
            ...
            return { ...body, color: value as string };
          }
        }
        return body;
      }),
