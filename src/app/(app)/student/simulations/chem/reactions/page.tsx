...
export default function ChemicalReactionsSimulation() {
  ...
  // Reaction library
  const reactions: { [key: string]: Reaction } = useMemo(() => ({
    combustion: {
  ...
  useEffect(() => {
    resetSimulation();
  }, [selectedReaction, particleCount]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  }, [isRunning, particles, temperature]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // Draw energy profile
  const drawEnergyProfile = useCallback((
    ...
    ctx.fillText("Products", endX - 40, productY - 5);
    ctx.fillText("Activation", startX + (endX - startX) / 2 - 20, activationY - 5);
  }, []);
